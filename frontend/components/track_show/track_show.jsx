...
class TrackShow extends React.Component {
  constructor(props) {
    ...
    this.displayAnnotationsAndLyrics = this.displayAnnotationsAndLyrics.bind(
      this
    );
  ...
  selectLyrics(e) {
    if (
      ...
      this.props.session.currentUser !== undefined
    ) {
      ...
      if (selectedLyrics.length > 0 && this.props.session.currentUser) {
        if (
          lyrics.anchorNode !== lyrics.focusNode ||
  ...
  selectLyric(e, anno) {
    if (
      ...
      this.props.openAnnotation(
        <AnnotationShow anno={anno} location={location} />
      );
    }
  }
    ...
    if (this.props.session.currentUser) {
      ...
      if (renderFavorite === true) {
        return (
          <Favorited
            size={35}
            onClick={() => this.removeFavorite(favoriteId)}
          />
        );
      } else {
        return <NotFavorited size={35} onClick={this.createFavorite} />;
      }
    }
  }
  ...
  createFavorite() {
    this.props.createFavorite({
      ...
      features: this.props.track.features,
      producer: this.props.track.producer,
    ...
    if (Object.keys(this.props.annotations).length !== 0) {
      ...
      lyrics.push(
        <span key={Math.random() + 4321} className="regular-lyrics">
          {this.props.track.lyrics.slice(
            startIdx,
            this.props.track.lyrics.length
          )}
        </span>
      );
    } else {
            ...
            <div className="black-layer">
              <div className="track-total-info">
                ...
                <div className="track-info-minus-photo">
                  ...
                  <div className="words-produced-by">
                    Produced by
                    <span className="producer">{track.producer}</span>
                  </div>
                  <div className="the-word-album">
                    Album
                    <Link
                      to={"/albums/" + album.id}
                      className="track-show-album-title"
                    >
                      {album.title}
                    </Link>
                  </div>
          ...
          <section className="track-lyrics">
            <div id="lyrics-container">
              <div id="the-lyrics-are-here">
                <p
                  id="the-lyrics-are-here"
                  onMouseDown={this.getStartLocation}
                  onMouseUp={this.selectLyrics}
                >
